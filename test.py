...
flags.DEFINE_string(
    "tfrecord_path",
    "/home/yanqinghao/code/3D-Printing-Project/dataset/input/record/train.record",
    ...
    with tf.Session() as sess:
        ...
        for i in range(10):
            ...
            (
                ...
                print_height,
                print_width,
            ) = sess.run([image, resized_image, label, height, width])
...
def read_tfrecord(
    tfrecord_path, num_samples=14635, num_classes=7, resize_height=800, resize_width=800
):
    keys_to_features = {
    ...
    resized_image = tf.squeeze(
        tf.image.resize_bilinear([image], size=[resize_height, resize_width])
    )
    return resized_image, label, image, height, width


def main():
    resized_image, label, image, height, width = read_tfrecord(
        tfrecord_path=FLAG.tfrecord_path,
